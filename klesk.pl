...
MAINLOOP: while (<STDIN>) {
	...
	if($victim_name) {
		...
		$write_string =~ s/$target_name_replace_string/$victim_name/g;
		print "cfg_taunt: \"$write_string\"\n" if DEBUG;
		...
		$write_string_rand_index = rand(scalar @write_string_list);
		$write_string = $write_string_list[$write_string_rand_index];
		$write_string =~ s/$target_name_replace_string/$enemy_name/g;
		print "cfg_insult: \"$write_string\"\n" if DEBUG;
		...
		$write_string =~ s/$target_name_replace_string/$enemy_name/g;
		print "cfg_praise: \"$write_string\"\n" if DEBUG;
		open (FILE, '>', $cfg_praise) or die "Unable to open cfg for writing! Exiting.";
		print FILE "say $write_string";
		close(FILE);
	} elsif($line =~ /(.*):/i) {
		# regex matches everything up to final colon, should survive weird post-steam names.
		$talker_name = $1;

		$write_string_rand_index = rand(scalar @insult_array_random);
		$write_string = $insult_array_random[$write_string_rand_index];
		$write_string =~ s/$target_name_replace_string/$talker_name/g;
		print "cfg_insult_talker: \"$write_string\"\n" if DEBUG;
